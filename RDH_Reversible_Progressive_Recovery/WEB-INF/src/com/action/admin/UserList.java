		...
		try
		{
				...
				if(com.user.util.Utility.parse1(request.getParameter("add1")).equals("YES"))
				{
					...
					if(!result)
					{
						...
						if(result)
						{
							...
							response.sendRedirect(request.getContextPath()+"/res/jsp/admin/adduser.jsp?no=1");
					}
					else
					{
						response.sendRedirect(request.getContextPath()+"/res/jsp/admin/adduser.jsp?no=1");
					...
					response.sendRedirect(request.getContextPath()+"/res/jsp/admin/adduser.jsp");
			}
			if(submit.equals("Edit"))
			{
				...
				else if(chk.length==1)
				{
					...
					request.setAttribute("rs", rs);
					rd=request.getRequestDispatcher("/res/jsp/admin/edituser.jsp");
					rd.forward(request,response);
				}
			}
