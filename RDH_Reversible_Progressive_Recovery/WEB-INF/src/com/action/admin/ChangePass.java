	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws IOException 
	{
		...
		try
		{
			...
			if(no==1)
			{
				...
				RequestDispatcher rd=request.getRequestDispatcher("/res/jsp/admin/changepass.jsp?no=0");
			...
			if(no==2)
			{
				...
				if(result)
				{
					if(npass.equals(cpass))
					{
						...
						{
							RequestDispatcher rd=request.getRequestDispatcher("/res/jsp/admin/changepass.jsp?no=3");
							rd.forward(request, response);
						}
					}
					else
					{
						RequestDispatcher rd=request.getRequestDispatcher("/res/jsp/admin/changepass.jsp?no=2");
						rd.forward(request, response);
					}
				}
				else
				{
					RequestDispatcher rd=request.getRequestDispatcher("/res/jsp/admin/changepass.jsp?no=1");
					rd.forward(request, response);
				}
			}
		}
